...
from gi.repository import Adw, Gio, Gtk  # type: ignore # noqa: E402

from constantes import DEFAULT_SOUNDFONT, INSTRUMENTOS  # noqa: E402
from modelo import EstadoMusical  # noqa: E402
from processador import MapeadorRegras  # noqa: E402
from servicos_audio import GeradorMIDI, PlayerAudio  # noqa: E402
...
class JanelaPrincipal(Adw.ApplicationWindow):
    ...
    def __init__(self, app):
        ...
        # Estado da aplicação
        self.estado_musical = EstadoMusical()
        self.mapeador = MapeadorRegras()
        self.gerador_midi = GeradorMIDI()
        self.player_thread = None
        self.soundfont_path = DEFAULT_SOUNDFONT
        self.caminho_txt_atual = None

        # Inicializar componentes lógicos
        self.lista_instrumentos_store = Gtk.StringList.new(
            [nome for _id, nome in INSTRUMENTOS]
        )

        # Caixa GTK principal
        box_principal = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self.set_content(box_principal)

        # Barra de cabeçalho
        self._construir_header(box_principal)

        # Sobreposição para notificações
        self.toast_overlay = Adw.ToastOverlay()
        self.toast_overlay.set_vexpand(True)
        box_principal.append(self.toast_overlay)

        # PreferencesPage como container principal
        prefs_page = Adw.PreferencesPage()
        self.toast_overlay.set_child(prefs_page)

        # Adicionar grupos de configuração e texto
        self._construir_painel_config(prefs_page)
        self._construir_editor_texto(prefs_page)

    def _construir_header(self, box_principal):
        header = Adw.HeaderBar()
        box_principal.append(header)
        ...
        # Ação 'Abrir'
        action_abrir_txt = Gio.SimpleAction.new('abrir_txt', None)
        action_abrir_txt.connect('activate', self._on_abrir_txt_clicked)
        ...
        self.btn_menu_salvar = Gtk.MenuButton.new()
        ...
        action_salvar_txt.connect('activate', self._on_salvar_txt_clicked)
        self.add_action(action_salvar_txt)

        # Ação 'Salvar MIDI'
        action_salvar_midi = Gio.SimpleAction.new('salvar_midi', None)
        action_salvar_midi.connect('activate', self._on_salvar_midi_clicked)
        ...
        # Botão 'Tocar'
        self.btn_tocar = Gtk.Button.new_from_icon_name('media-playback-start-symbolic')
        ...
        # Botão 'Parar'
        self.btn_parar = Gtk.Button.new_from_icon_name('media-playback-stop-symbolic')
    ...
    def _construir_painel_config(self, prefs_page):
        ...
        # Instrumento
        self.combo_row_inst = Adw.ComboRow()
    ...
    def _construir_editor_texto(self, prefs_page):
    ...
    def _atualizar_estado_dos_controles(self):
    ...
    def _on_tocar_clicked(self, widget):
        """Iniciar a reprodução em uma thread."""
        # Obter texto
        self._atualizar_estado_dos_controles()
        start_iter = self.text_buffer.get_start_iter()
        end_iter = self.text_buffer.get_end_iter()
        texto = self.text_buffer.get_text(start_iter, end_iter, False)
        ...
        self.player_thread = PlayerAudio(
            self.soundfont_path, eventos, estado_para_playback
        )
    ...
    def _on_parar_clicked(self, widget):
        """Solicitar que a thread do player pare."""
        if self.player_thread and self.player_thread.is_alive():
            self.player_thread.parar()

    def _on_playback_terminado(self):
        """Callback executado na thread principal quando o player termina."""
        self.player_thread = None
        self.btn_tocar.set_sensitive(True)
        self.btn_parar.set_sensitive(False)

    def _on_abrir_txt_clicked(self, action, param):
        ...
        dialog.add_buttons(
            '_Cancelar', Gtk.ResponseType.CANCEL, '_Abrir', Gtk.ResponseType.OK
        )
        ...
        dialog.connect('response', self._on_abrir_txt_response)
        dialog.show()

    def _on_abrir_txt_response(self, dialog, response):
        if response == Gtk.ResponseType.OK:
            try:
                self.caminho_txt_atual = dialog.get_file().get_path()
                with open(self.caminho_txt_atual) as f:
                    conteudo = f.read()
    ...
    def _on_salvar_txt_clicked(self, action, param):
        ...
        dialog.add_buttons(
            '_Cancelar', Gtk.ResponseType.CANCEL, '_Salvar', Gtk.ResponseType.OK
        )

        if self.caminho_txt_atual:
            dialog.set_file(Gio.File.new_for_path(self.caminho_txt_atual))
        else:
            dialog.set_current_name('musica.txt')

        dialog.connect('response', self._on_salvar_txt_response)
        dialog.show()

    def _on_salvar_txt_response(self, dialog, response):
        if response == Gtk.ResponseType.OK:
            try:
                caminho = dialog.get_file().get_path()
                start_iter = self.text_buffer.get_start_iter()
                end_iter = self.text_buffer.get_end_iter()
                conteudo = self.text_buffer.get_text(start_iter, end_iter, False)
                with open(caminho, 'w') as f:
                    f.write(conteudo)
                self.caminho_txt_atual = caminho
                self.set_title(f'Gerador de Música - {self.caminho_txt_atual}')
                self._show_toast('Arquivo de texto salvo.')
            except Exception as e:
                self._show_toast(f'Erro ao salvar texto: {e}')
        dialog.destroy()

    def _on_salvar_midi_clicked(self, action, param):
        ...
        dialog.connect(
            'response', self._on_salvar_midi_response, eventos, estado_para_midi
        )
        dialog.show()

    def _on_salvar_midi_response(self, dialog, response, eventos, estado_para_midi):
        """Callback que efetivamente salva o MIDI."""
        if response == Gtk.ResponseType.OK:
            try:
                caminho = dialog.get_file().get_path()
                if not (caminho.endswith('.mid') or caminho.endswith('.midi')):
                    caminho += '.mid'

                sucesso = self.gerador_midi.gerar_e_salvar(
                    eventos, estado_para_midi, caminho
                )
                if not sucesso:
                    raise Exception('Ocorreu um erro interno no GeradorMIDI.')
...
class Aplicacao(Adw.Application):
    ...
    def __init__(self, **kwargs):
        super().__init__(application_id='br.ufrgs.inf01120.fase3', **kwargs)
        self.window = None
